    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">Pendentes</p>
          <p className="text-[22px] font-bold text-amber-600 mt-1">{stats.pendentes}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">Aprovados</p>
          <p className="text-[22px] font-bold text-green-600 mt-1">{stats.aprovados}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wide">Rejeitados</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4">
          ...
          <p className="text-[22px] font-bold text-slate-800 mt-1">{stats.total}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="p-4 border-b border-slate-100">
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              className="flex-1 px-3 py-2 rounded-lg border border-slate-200 text-[13px] text-slate-700 focus:ring-1 focus:ring-primary/30 focus:border-primary/50 transition-colors"
            />
            <select
              value={filtroEstado}
              onChange={function (e) { setFiltroEstado(e.target.value); carregar(1); }}
              className="px-3 py-2 rounded-lg border border-slate-200 text-[13px] text-slate-700 focus:ring-1 focus:ring-primary/30 focus:border-primary/50 transition-colors"
            >
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <div className="p-8 text-center text-[13px] text-slate-400">A carregar...</div>
          ) : pedidos.length === 0 ? (
            <div className="p-8 text-center text-[13px] text-slate-400">Sem pedidos</div>
          ) : (
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>
                  <th className="px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase">Colaborador</th>
                  <th className="px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase">Tipo</th>
                  <th className="px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase">Título</th>
                  <th className="px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase">Estado</th>
                  <th className="px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase">Quando</th>
                  <th className="px-4 py-2.5 text-[11px] font-semibold text-slate-500 uppercase text-right">Ações</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors">
                      <td className="px-4 py-3">
                        <p className="text-[13px] font-medium text-slate-700">{colab.nome_completo || "—"}</p>
                        <p className="text-[11px] text-slate-400">{colab.numero_colaborador || ""}</p>
                      </td>
                      <td className="px-4 py-3">
                        <span className="text-[12px] text-slate-600">{TIPO_LABELS[p.tipo] || p.tipo}</span>
                      </td>
                      <td className="px-4 py-3 text-[13px] text-slate-700 font-medium max-w-[200px] truncate">{p.titulo}</td>
                      <td className="px-4 py-3">
                        <span className={"text-[11px] font-semibold px-2 py-0.5 rounded-full " + estadoClasses(p.estado)}>
                          {p.estado.charAt(0).toUpperCase() + p.estado.slice(1)}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-[12px] text-slate-400">{timeAgo(p.createdAt)}</td>
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={function () { handleVerDetalhe(p); }}
                            className="px-2.5 py-1 rounded-lg text-[11px] font-medium text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors"
                          >
                            Ver
                          </button>
                          {p.estado === "pendente" && (
          ...
          <div className="p-4 border-t border-slate-100 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button
                ...
                className="px-3 py-1 rounded-lg border border-slate-200 text-[12px] text-slate-600 hover:bg-slate-50 disabled:opacity-30 transition-colors"
              >
                Anterior
              </button>
              <button
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-100 px-5 py-4 flex items-center justify-between rounded-t-xl">
              <h3 className="text-[15px] font-semibold text-slate-800">Detalhe do Pedido</h3>
              <button onClick={function () { setShowDetalhe(false); }} className="text-slate-400 hover:text-slate-600 text-[14px]">
                Fechar
              </button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                ...
                <p className="text-[15px] font-semibold text-slate-800 mt-0.5">{pedidoDetalhe.titulo}</p>
                <p className="text-[12px] text-slate-400">{TIPO_LABELS[pedidoDetalhe.tipo] || pedidoDetalhe.tipo}</p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-[11px] text-slate-400 uppercase tracking-wide">Colaborador</p>
                  <p className="text-[13px] font-medium text-slate-700 mt-0.5">{(pedidoDetalhe.colaborador || {}).nome_completo || "—"}</p>
                </div>
                <div>
                  <p className="text-[11px] text-slate-400 uppercase tracking-wide">Estado</p>
                ...
                <div>
                  <p className="text-[11px] text-slate-400 uppercase tracking-wide">Criado em</p>
                  <p className="text-[13px] text-slate-600 mt-0.5">{formatDate(pedidoDetalhe.createdAt)}</p>
                </div>
                {pedidoDetalhe.responded_at && (
                  <div>
                    <p className="text-[11px] text-slate-400 uppercase tracking-wide">Respondido em</p>
                    <p className="text-[13px] text-slate-600 mt-0.5">{formatDate(pedidoDetalhe.responded_at)}</p>
                  </div>
                )}
              </div>
              ...
              {pedidoDetalhe.descricao && (
                <div>
                  <p className="text-[11px] text-slate-400 uppercase tracking-wide">Descrição</p>
                  <p className="text-[13px] text-slate-600 mt-0.5">{pedidoDetalhe.descricao}</p>
                </div>
              )}
              ...
              {pedidoDetalhe.tipo === "ferias" && dadosDetalhe.data_inicio && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <p className="text-[11px] text-slate-400 uppercase tracking-wide">Data Início</p>
                    <p className="text-[13px] text-slate-600 mt-0.5">{formatDate(dadosDetalhe.data_inicio)}</p>
                  </div>
                  <div>
                    <p className="text-[11px] text-slate-400 uppercase tracking-wide">Data Fim</p>
                    <p className="text-[13px] text-slate-600 mt-0.5">{formatDate(dadosDetalhe.data_fim)}</p>
                  </div>
                </div>
              )}
              ...
              {pedidoDetalhe.tipo === "adiantamento" && dadosDetalhe.valor && (
                <div>
                  <p className="text-[11px] text-slate-400 uppercase tracking-wide">Valor</p>
                  <p className="text-[15px] font-semibold text-slate-700 mt-0.5">{Number(dadosDetalhe.valor).toLocaleString("pt-AO")} AOA</p>
                </div>
              )}

              {pedidoDetalhe.comentario && (
                <div>
                  <p className="text-[11px] text-slate-400 uppercase tracking-wide">Comentário</p>
                  <p className="text-[13px] text-slate-600 mt-0.5">{pedidoDetalhe.comentario}</p>
                </div>
              )}
              ...
              {pedidoDetalhe.estado === "pendente" && (
                <div className="pt-3 border-t border-slate-100">
                  <textarea
                    ...
                    className="w-full px-3 py-2 rounded-lg border border-slate-200 text-[13px] text-slate-700 focus:ring-1 focus:ring-primary/30 focus:border-primary/50 transition-colors resize-none mb-3"
                  />
